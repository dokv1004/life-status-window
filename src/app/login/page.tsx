'use client';

import { signInWithPopup, GoogleAuthProvider } from 'firebase/auth';
import { auth } from '@/lib/firebase';
import { useRouter } from 'next/navigation';
import { useAuth } from '@/hooks/useAuth';
import { useEffect } from 'react';
import LoginSkeleton from '@/components/LoginSkeleton';

export default function LoginPage() {
  const router = useRouter();
  const { user, loading } = useAuth();

  useEffect(() => {
    if (user) {
      router.push('/');
    }
  }, [user, router]);

  const handleGoogleLogin = async () => {
    try {
      const provider = new GoogleAuthProvider();
      await signInWithPopup(auth, provider);
      router.push('/');
    } catch (error) {
      console.error('Login error:', error);
      alert('로그인 중 오류가 발생했습니다.');
    }
  };

  if (loading) {
    return <LoginSkeleton />;
  }

  return (
    <main className="flex flex-col items-center justify-center min-h-screen md:min-h-[calc(100vh-4rem)] p-4 pb-20 md:pb-4 bg-gray-50/50">
      {/* 로그인 카드 */}
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg border border-blue-100 overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
        {/* 헤더 */}
        <div className="p-8 text-center space-y-2">
          <h1 className="text-2xl font-bold text-blue-900">
            길드 접수처
          </h1>
          <p className="text-gray-500">
            모험을 시작하려면 신원을 증명해주세요.
            <br />
            <span className="text-sm">(구글 계정으로 3초 만에 등록됩니다)</span>
          </p>
        </div>

        {/* 구글 로그인 버튼 */}
        <div className="px-8 pb-8">
          <button
            onClick={handleGoogleLogin}
            className="w-full h-12 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-all flex items-center justify-center gap-3 font-medium active:scale-95"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path
                fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
              />
              <path
                fill="#34A853"
                d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
              />
              <path
                fill="#FBBC05"
                d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
              />
              <path
                fill="#EA4335"
                d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
              />
            </svg>
            Google로 계속하기
          </button>
        </div>
      </div>

      {/* 하단 약관 문구 */}
      <p className="mt-6 text-xs text-gray-400 text-center animate-in fade-in slide-in-from-bottom-4 duration-1000">
        로그인 시 이용약관 및 개인정보처리방침에 동의하게 됩니다.
      </p>
    </main>
  );
}
