...
export async function POST(request: NextRequest) {
  try {
    ...
    const requestIndex = myReceivedRequests.findIndex(r => r.uid === friendUid);

    if (requestIndex === -1) {
      return NextResponse.json(
        { error: '해당 친구 요청을 찾을 수 없습니다.' },
        { status: 404 }
      );
    }

    // 친구 객체 생성
    const myNewFriend: Friend = {
      uid: friendUid,
    ...
    // 내 정보 업데이트: 친구 추가 + 받은 요청 제거
    await updateDoc(myRef, {
      friends: arrayUnion(myNewFriend),
      friendRequestsReceived: updatedReceivedRequests,
    });

    // 상대방 정보 업데이트: 친구 추가 + 보낸 요청 제거
    await updateDoc(friendRef, {
      friends: arrayUnion(theirNewFriend),
